...
export async function GET(request: NextRequest) {
  ...
  if (!filePath || !siteId || !driveId) {
    ...
    if (sharePointUrl) {
      ...
      return NextResponse.json({ 
        error: 'Video file needs to be re-uploaded to work with the new system. Please upload the video again.' 
      }, { status: 400 });
    }
  ...
  try {
    console.log('Video proxy request:', { filePath, siteId, driveId });
    ...
    return NextResponse.json({ error: 'Failed to proxy video file' }, { status: 500 });
  }
}
